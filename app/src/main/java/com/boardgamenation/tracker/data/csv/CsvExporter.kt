...
class CsvExporter @Inject constructor(
    ...
    private suspend fun buildTables(): List<Pair<String, Table>> {
        ...
        val gameExpansions = gameDao.getAllExpansionLinks()
            .sortedWith(compareBy({ it.expansionId }, { it.baseGameId }))
        ...
        val sessionObjectives = sessionDao.getAllSessionObjectives()
            .sortedWith(compareBy({ it.sessionId }, { it.sortOrder }, { it.objective }))
        ...
        return listOf(
            ...
            CsvSchema.GAME_EXPANSIONS to Table(
                CsvSchema.gameExpansionColumns,
                gameExpansions.map {
                    listOf(it.expansionId.toString(), it.baseGameId.toString())
                }
            ),
            ...
            CsvSchema.SESSION_OBJECTIVES to Table(
                CsvSchema.sessionObjectiveColumns,
                sessionObjectives.map {
                    listOf(
                        it.sessionId.toString(),
                        it.objective,
                        it.hintsUsed.toString(),
